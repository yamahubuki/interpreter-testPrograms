...
- StmtListで無限ループし、複数の要素(ここでは全てNL)を取得・表示できる事の確認
...
- 10-4では、文字列の比較が正しく実装されている事の確認。もし「あ」が出力されている場合、文字列をJavaの==演算子で比較してしまっている可能性が高い。
- 実行結果
	- 10
		- 10
	- 10-2
...
- 式の左から順に計算すれば正しい答えを得られるテストケースです。
- 実行結果
	- 21
		- 4
	- 21-2
